...
const Circle = forwardRef(({ className, children }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        "z-10 flex items-center justify-center rounded-full border-2 p-3 shadow-[0_0_20px_-12px_rgba(0,0,0,0.8)]",
        className
      )}
    >
      {children}
    ...
    <div className="flex-1 p-6">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-muted-foreground">
          Leverage multiple AI models for enhanced accuracy
        </p>
      </div>
      
      <div
        className="relative flex h-[400px] items-center justify-center overflow-hidden rounded-xl"
        ref={containerRef}
      >
        <div className="flex w-full flex-col max-w-md max-h-[200px] items-stretch justify-between gap-10">
          <div className="flex flex-row items-center justify-between">
            <Circle ref={div1Ref} className="size-14 bg-white">
              <Icons.openai />
            </Circle>
            <Circle ref={div5Ref} className="size-14 bg-gradient-to-br from-blue-50 to-blue-100">
              <Image src={gemma} alt="gemma" width={32} height={32} />
            </Circle>
          </div>
          
          <div className="flex flex-row items-center justify-center">
            <Circle ref={div4Ref} className="size-20 bg-gradient-to-br from-slate-800 to-slate-900">
              <Icons.qs className="text-white" />
            </Circle>
          </div>
          
          <div className="flex flex-row items-center justify-between">
            <Circle ref={div2Ref} className="size-14 bg-white">
              <Image src={metaAI2} alt="meta" width={32} height={32} />
            </Circle>
            <Circle ref={div3Ref} className="size-14 bg-white">
              <Image src={mixtral} alt="mixtral" width={32} height={32} />
            </Circle>
          </div>
        </div>
  ...
  return (
    <div className="flex-1 p-6">
      ...
      <div
        ...
        ref={containerRef}
      >
        <div className="flex w-full max-w-md flex-row items-stretch justify-between gap-10">
          <div className="flex flex-col justify-center">
            <Circle ref={div7Ref} className="size-14 bg-gradient-to-br from-slate-800 to-slate-900">
              <Icons.user className="text-white" />
            </Circle>
          </div>
          
          <div className="flex flex-col justify-center">
            <Circle ref={div6Ref} className="size-20 bg-gradient-to-br from-slate-800 to-slate-900">
              <Icons.qs className="text-white" />
            </Circle>
          </div>
          ...
          <div className="flex flex-col justify-center gap-3">
            {[
              ...
              { ref: div4Ref, src: oracle, alt: "oracle" },
              { ref: div5Ref, src: neonDB, alt: "neonDB" }
            ].map((db, index) => (
              <Circle key={index} ref={db.ref} className="size-14 bg-white hover:bg-blue-50 transition-colors">
                <Image src={db.src} alt={db.alt} width={32} height={32} />
              </Circle>
            ))}
          </div>
        </div>
